...
- Если запрос пользователя неоднозначен и ты не можешь определить нужные данные, используй `ask_user` чтобы задать уточняющий вопрос. Но старайся действовать самостоятельно, когда контекст достаточен.
...
def build_system_prompt(categories_text: str, knowledge_text: str = "",
    ...
    knowledge_section = ""
    if knowledge_text:
        knowledge_section = f"\n## База знаний (сохранённые знания)\n\n{knowledge_text}\n"
    custom_section = ""
