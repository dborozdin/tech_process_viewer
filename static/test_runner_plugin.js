...
(function () {
  ...
  function rowHTML(r) {
    const cls = r.status === "PASS" ? "color:#28a745" : (r.status === "FAIL" ? "color:#cc3333" : "color:#999");
    ...
    return `<tr>
      ...
      <td style="padding:4px 6px;text-align:center"><b style="${cls}">${esc(r.status)}</b></td>
  ...
  function renderSettings(meta, groupKey) {
    ...
    const link = `<div style="margin-top:8px"><a href="/static/test_API_settings_view" target="_blank" onclick="event.preventDefault(); window.open('/openapi/test_API_settings.json','_blank')">📋 Открыть полный test_API_settings.json</a></div>`;
    return dbBlock + scBlock + link;
  }
